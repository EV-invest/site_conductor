    ...
    flake-utils.lib.eachDefaultSystem (
      ...
      let
        ...
        github = v_flakes.github {
          ...
          gitignore.extra = ''
            ## frontend/public/ is 100% generated by `nix run` (populate-docs): source imagery lives in frontend/assets/, while docs/mfe/logo/icons are built from flake inputs. Nothing under public/ is source.
            frontend/public/
            ## Node / Next.js
            node_modules/
            .next/
        ...
        # Built with the dev nightly toolchain (the workspace pins `cargo-features = ["codegen-backend"]`, which stable cargo rejects).
        # `aws-lc-sys` (via reqwest/rustls) needs cmake + libclang; `openssl-sys` (via sentry's native-tls) needs openssl + pkg-config; `.cargo/config` links with mold.
        rustPlatform = pkgs.makeRustPlatform { cargo = rust; rustc = rust; };
        backendSrc = pkgs.lib.cleanSourceWith {
          src = ./.;
          # Cargo only needs the workspace crates + manifests; drop the heavy sibling trees (frontend, target, local DB, vcs) from the build input.
          filter = path: _type:
        ...
        backendBin = rustPlatform.buildRustPackage {
          ...
          # Drop the `swagger` feature: utoipa-swagger-ui's build script fetches the UI bundle over the network, which the sandbox forbids.
        ...
        # `output: "standalone"` (next.config.ts) emits a self-contained `.next/standalone/server.js` that runs under plain node — no npm at runtime.
        # The committed brand assets are baked into public/; the runtime best-effort docs (whitepaper/blog/portfolio MFE, built from private sibling clones by populate-docs) are intentionally NOT in the image, so those pages degrade — same contract the dev runner already tolerates.
        frontendApp = pkgs.buildNpmPackage {
          ...
          # `npm run build` chains stylelint (a CI lint gate, not an image concern); call next build directly so a style nit can't fail the image.
          # public/ is generated, not committed — stage the brand assets it needs first.
          buildPhase = ''
            runHook preBuild
        ...
        # Backend and frontend are separate containers (not one): distinct runtimes/processes, and so each rolls out + restarts independently — a frontend crash or deploy never touches the API.
        ...
        # ── shared shims ────────────────────────────────────────────────────
        # rust-lld (wasm32 linker) embeds the wrong rpath on macOS — it looks for libLLVM.dylib in bin/../lib/ but Nix puts it one level up in lib/. The FALLBACK var only kicks in when normal resolution fails.
        # Kept ready for the Rust backend that grows under backend/.
        ...
        # Populate frontend/public/ with the logo (from the public ev_assets input) and the typst-built whitepaper/blog HTML+PDF.
        # The docs come from the LOCAL sibling clones (../whitepaper, ../blog) built at runtime — token-free (those repos are private, but their flake inputs are public) and best-effort: a missing clone or a failed build only warns, so the frontend always boots and the doc pages degrade to a graceful "unavailable"/last-good state. Idempotent (nix caches the builds).
        populateDocs = pkgs.writeShellApplication {
          ...
          text = ''
            ...
            # public/ is 100% generated; the only committed source is frontend/assets/ (section imagery). Stage it plus the brand logo (also the manifest + JSON-LD icon) into the served public/assets/.
            ...
            # Real-estate MFE bundle (the `embeds` artifact from the sibling REA repo), baked into public/ and served same-origin at /mfe — the host owns the bundle's static assets; REA's container no longer serves it.
            mfe=""
            ...
            # nix-store copies land read-only; make them writable so a later run can overwrite. Tolerate an empty glob.
            chmod -f u+w "$pub"/whitepaper.* "$pub"/blogs/* 2>/dev/null || true
          '';
        };

        # ── frontend (Next.js marketing site) ───────────────────────────────
        # Standalone npm app under frontend/. `npm install` generates/updates the lockfile and installs into frontend/node_modules on first run.
        ...
        # ── backend (Axum API) ──────────────────────────────────────────────
        # Needs a Postgres (use `.#db` or `.#dev`). Every other integration (SMTP, Sentry, PostHog) is a no-op until its env is set, so the bare default just boots against the local trust-auth cluster.
        ...
        # ── local Postgres ──────────────────────────────────────────────────
        # Project-local dev database. Cluster data + unix sockets live under .pg/ at the repo root (gitignored). First run initdb's a trust-auth cluster and creates `ev_landing`; later runs just start the server. Listens on 127.0.0.1:5432 — matches backend/.env.example.
        ...
        # ── contract codegen ────────────────────────────────────────────────
        # `nix run .#gen-api` → dump the Rust/utoipa OpenAPI spec to backend/openapi.json, then regenerate the TS client from it. Run after changing any DTO or handler signature; commit both the spec and client.
        ...
        # ── dev orchestrator ────────────────────────────────────────────────
        # `nix run .#dev` → Postgres + backend + frontend. Postgres starts first; the backend launches once it accepts connections. One trap tears the whole tree down on Ctrl-C / exit.
      ...
      in
      {
        ...
        devShells.default =
          ...
          mkShell {
            ...
            # Playwright (frontend visual tests): drive the nixpkgs browsers instead of the npm-downloaded ones (those dynamically link libs absent on NixOS).
            # The frontend `@playwright/test` version MUST match playwright-driver's or the browser revisions won't line up.
